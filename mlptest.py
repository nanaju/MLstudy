...
def test_mlp(learning_rate=0.01, L1_reg=0.00, L2.reg=0.0001, n_epochs=1000, dataset='mnist.pkl.gz', batch_size=20, n_hidden=500): #なぜclassじゃない？　epochsとは？
	...
	#ミニバッチの数
	n_train_batches = train_set_x.get_value(bollow=True).shape[0] / batch_size
	n_valid_batches = valid_set_x.get_value(bollow=True).shape[0] / batch_size
	n_test_batches = test_set_x.get_value(bollow=True).shape[0] / batch_size
	...
	#コスト関数を計算するシンボル
	cost = classifier.negative_log_likelihood(y) + L1_reg*classifier.L1 + L2_reg * classifier.L2.sqr
